...
const PROFILE_SEED_REVISION = 4;
const RESULTS_SEED_REVISION = 4;
const FEES_SEED_REVISION = 4;
const TIMETABLE_SEED_REVISION = 5;
const RECEIPTS_SEED_REVISION = 7;
const ATTENDANCE_SEED_REVISION = 2;
...
const ARYAN_PROFILE = {
  ...
  father: 'SANTOSH KUMAR', mother: 'JYOTI KUMARI', dob: '17 March 2006', phone: '7011408281',
  address: 'Shadipur, New Delhi, Delhi - 110008', localAddress: 'Shadipur, New Delhi, Delhi - 110008', permanentAddress: 'Shadipur, New Delhi, Delhi - 110008', city: 'New Delhi', state: 'Delhi', pincode: '110008', country: 'India', className: 'Btech CSE IV 2023-2024 Sem VII', programme: 'Bachelor of Technology in Computer Science and Engineering',
...
const DEFAULT = {
  profile: {
    ...
    father: 'SANTOSH KUMAR', mother: 'JYOTI KUMARI', dob: '17 March 2006', nationality: 'Indian', phone: '7011408281', gender: 'Male', religion: 'Hinduism',
    admission: '23SCSE1012155', application: 'UG2323717', className: 'Btech CSE IV 2023-2024 Sem VII', programme: 'Bachelor of Technology in Computer Science and Engineering', address: '', localAddress: '', permanentAddress: '', city: '', state: '', pincode: '', country: 'India', abcId: '',
    avatar: '', ...ARYAN_PROFILE
  },
  attendanceRevision: ATTENDANCE_SEED_REVISION,
  attendance: [
    { title: 'Research Methodology and IPR', code: 'R1UC701T', type: 'PP', attended: 9, delivered: 10 },
    { title: 'Generative and Explainable AI', code: 'R1UD702B', type: 'PP', attended: 9, delivered: 10 }
  ],
  ...
  receipts: [{
    number: '2026-2027/GU//13419', date: '14/09/2026', amount: 164000,
...
function mergeDefaults(saved) {
 ...
 const attendance = (saved.attendanceRevision || 0) < ATTENDANCE_SEED_REVISION ? structuredClone(DEFAULT.attendance) : (saved.attendance || structuredClone(DEFAULT.attendance));
 return { ...structuredClone(DEFAULT), ...saved, profile, fees, attendance, attendanceRevision: ATTENDANCE_SEED_REVISION, receipts, receiptRevision: RECEIPTS_SEED_REVISION, resultsRevision: RESULTS_SEED_REVISION, results, records: { ...DEFAULT.records, ...(saved.records || {}) }, timetableRevision: TIMETABLE_SEED_REVISION, timetable: savedClasses.map(item => ({ ...item, date: item.date || '2026-09-13' })) };
}
...
function feesPage() {
 ...
 return `<section class="shell">${topbar('Fees')}<section class="hero-blue"><h1 style="font-size:25px">Fees</h1><p style="margin:6px 0 0;font-size:19px;font-weight:800">My Fees</p></section><main class="content-overlap"><section class="card identity-card"><div class="identity-head">${avatar('small-avatar')}<div><h2>${escapeHtml(state.profile.name)}</h2><p>Admission No.</p><strong>${escapeHtml(state.profile.admission)}</strong><p>Class</p><strong>${escapeHtml(state.profile.className)}</strong></div></div><div class="info-rows"><div><p>Division</p><strong>${escapeHtml(state.profile.section)}</strong></div><div><p>Roll No.</p><strong>${escapeHtml(state.profile.roll)}</strong></div><div><p>PRN</p><strong>${escapeHtml(state.profile.roll)}</strong></div></div><div class="programme"><p>Programme</p><strong>${escapeHtml(state.profile.programme)}</strong></div></section><div class="fee-menu">${heads.map(n=>`<button class="fee-head ${activeFee===n?'active':''}" data-fee="${n}"><span>${icon(n)}</span>${n}${n==='All Fees'?'<small style="display:block;color:#6e7278;margin-top:4px">(Incld. Misc)</small>':''}</button>`).join('')}</div><section class="card outstanding"><span>Total Outstanding Amount</span><strong>${money(outstanding)}</strong></section><div class="fee-summary">${[['Academic',outstanding],['Academic Miscellaneous',0],['Hostel',0],['Hostel Miscellaneous',0],['Transport',0],['Transport Miscellaneous',0]].map(([l,v])=>`<section class="card">${l}<strong>${money(v)}</strong></section>`).join('')}</div>${receiptCard}</main>${bottomNav()}</section>`;
}
